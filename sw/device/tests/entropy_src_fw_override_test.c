...
enum {
  ...
  kBitsPerSample = 4,
  /**
   * Timeout for operations which wait for random data.
   */
  kTimeoutUsec = 50000,
  /**
...
void ottf_external_isr(uint32_t *exc_info) {
  ...
  if (words_to_output > 0) {
    ...
    // Start the SHA3 block ready for us to start inserting into the FIFO.
    ...
    // Disable the SHA3 block, allowing it to finish processing and pass the
    // data through to the output of the entropy source.
    CHECK_DIF_OK(dif_entropy_src_fw_override_sha3_start_insert(
...
status_t firmware_override_extract_insert(
  ...
  for (size_t i = 0; i < 16; i++) {
    TRY(rv_core_ibex_testutils_get_rnd_data(&rv_core_ibex, kTimeoutUsec,
                                            &data));
  }
