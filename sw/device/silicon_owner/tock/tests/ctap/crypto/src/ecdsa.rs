...
impl Signature {
    ...
    pub fn to_bytes(&self, bytes: &mut [u8; Signature::BYTES_LENGTH]) {
        self.r
            .to_int()
            .as_bin(array_mut_ref![bytes, 0, int256::NBYTES]);
        self.s
            .to_int()
            .as_bin(array_mut_ref![bytes, int256::NBYTES, int256::NBYTES]);
    }
}
...
impl PubKey {
    ...
    pub fn to_coordinates(&self, x: &mut [u8; NBYTES], y: &mut [u8; NBYTES]) {
        self.p.getx().to_int().as_bin(x);
        self.p.gety().to_int().as_bin(y);
    }
    ...
    H: Hash256,
{
    pub fn new(sk: &SecKey, msg: &[u8]) -> Rfc6979<H> {
        ...
        marker[0] = 0x00;
        Int256::as_bin(&sk.k.to_int(), contents_k);
        Int256::as_bin(&Int256::from_bin(&h1).modd(&Int256::N), contents_h1);
