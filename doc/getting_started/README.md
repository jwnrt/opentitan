...
We officially support Ubuntu 22.04 LTS which is where we run our automated tests.
...
To install Python dependencies with *either* pip or uv:

```sh
pip3 install --user -r python-requirements.txt --require-hashes
uv venv --python 3.9 && uv pip install -r python-requirements.txt --require-hashes
```

The `--user` flag of pip installs dependencies to `~/.local/bin` by default.
